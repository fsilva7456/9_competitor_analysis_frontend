...
// API URLs for each service - ensure these include https:// in the environment variables
  ...
  const runAnalysis = async (endpoint: string, loadingKey: LoadingKey) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData?.detail || `${loadingKey} analysis failed (${response.status})`)
      }
